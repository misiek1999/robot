...
mq_log_data_t rec_data;    // Buffer for income data
...
void save_data_to_file(){
    ...
    int status = mq_timedreceive(mes_to_logger_queue, (char *)&rec_data[0], sizeof(mq_log_data_t), NULL, &rec_timeout);
...
void write_to_log(const  char* str_to_log){
    ...
    // Catch error code
    if (status < 0 )
        std::cerr<<"MQ LOG SEND ERROR: "<<status<<" -> "<< strerror(errno) <<std::endl;
}